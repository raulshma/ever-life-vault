...
const colorClassMap = {
  ...
  purple: 'bg-[hsl(var(--primary)/0.15)] text-[hsl(var(--primary))]',
  orange: 'bg-[hsl(var(--warning)/0.20)] text-[hsl(var(--warning))]'
} as const;
      ...
      {stats.map((stat, index) => {
        ...
        return (
          <Card key={index} className="glass shadow-card shine-card hover-lift">
            <CardContent className="p-6 text-center">
              <div className={`flex items-center justify-center w-12 h-12 rounded-lg mx-auto mb-3 ${colorClassMap[stat.color as keyof typeof colorClassMap]}`}>
