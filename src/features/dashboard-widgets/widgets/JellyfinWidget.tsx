      ...
      {!config.serverUrl || !config.apiKey ? (
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center justify-between">
            <div>Server: {info?.ServerName || '(unknown)'} {info?.Version && <span className="text-muted-foreground">v{info.Version}</span>}</div>
            <Button size="sm" variant="outline" onClick={load}>Refresh</Button>
          </div>
          <div>Active sessions: {sessions.filter(s => s.IsActive).length}</div>
